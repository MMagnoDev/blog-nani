...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Nosso espaço", href: "/nosso-espaco" },
    { name: "Blog", href: "/blog" },
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-8 md:px-12 lg:px-20 h-16 sm:h-20 flex items-center justify-between">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`relative text-xs uppercase tracking-wider font-semibold transition-colors duration-300 py-1 group ${
                  active ? "text-[#6F7A69]" : "text-[#656C60] hover:text-[#6F7A69]"
                }`}
              >
                <span>{link.name}</span>
                <span
                  className={`absolute bottom-0 left-0 w-full h-[1.5px] bg-[#6F7A69] origin-left transition-transform duration-300 ease-out ${
        ...
        <div className="hidden lg:block">
          <Button
            ...
            variant="primary"
            className="px-5 py-2 text-xs uppercase tracking-wider font-semibold hover:shadow-md transition-all duration-300"
          >
            Agendar
          </Button>
        </div>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={(e) => handleNavClick(e, link.href)}
                  className={`text-lg transition-colors py-2 border-b border-[#E5E4DB]/50 flex items-center justify-between ${
                    active ? "font-semibold text-[#6F7A69]" : "font-medium text-[#495045] hover:text-[#6F7A69]"
                  }`}
                >
                  <span>{link.name}</span>
